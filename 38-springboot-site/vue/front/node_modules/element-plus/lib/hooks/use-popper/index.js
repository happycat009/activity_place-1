...
const usePopperHook = () => {
  ...
  const popperStyle = vue.ref({ zIndex: popupManager.PopupManager.nextZIndex() });
  const visible = vue.ref(false);
  const isManual = vue.computed(() => props.manualMode || props.trigger === "manual");
  const popperId = `el-popper-${util.generateId()}`;
  let popperInstance = null;
  const { renderTeleport, showTeleport, hideTeleport } = index.useTeleport(popupRenderer, vue.toRef(props, "appendToBody"));
  const { show, hide } = index$1.useModelToggle({
    indicator: visible,
    onShow,
    onHide
  });
  const { registerTimeout, cancelTimeout } = index$2.useTimeout();
  function onShow() {
    popperStyle.value.zIndex = popupManager.PopupManager.nextZIndex();
